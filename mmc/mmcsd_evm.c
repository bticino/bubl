		      ...
		      Uint8 checkStatus, Uint16 statusBits )
{
	...
	MMCSD_MMCARGHL =
		( MMCSD_MMCARGHL & 0xFFFF0000 ) | ( argument & 0x0000FFFF );
	MMCSD_MMCARGHL =
		( MMCSD_MMCARGHL & 0x0000FFFF ) | ( argument & 0xFFFF0000 );
	...
	if ( checkStatus )
	{
		...
		return ( ( statusRegBits & statusBits ) == statusBits )
			? 0 : E_DEVICE;
	}
...
/*
 *  Parse the Card Status value, sent to the MMC/SD card, in response to cmd
 */
...
Uint16 MMCSD_getCardCSD( MMCSD_ResponseData* mmcsdResponse,
			 Uint16* cardCSD, MMCSD_csdRegInfo* localCSDInfo )
{
	Uint16 i;
	...
	// Extracting 7 bits: For MMC - 5 bits ; For SD - 7 bits
...
Uint16 MMCSD_readNWords( Uint32* data, Uint32 numofBytes, Uint32 cardMemAddr )
{
	...
	/* Check if the FIFO Threshold default( 32bytes ), or set to 64 */
	if ( fifoThrlevel )
	{
...
Uint16 MMCSD_writeNWords( Uint32* data, Uint32 numofBytes, Uint32 cardMemAddr )
{
	...
	/* Check the FIFO Threshold is not as default (32) set it to 64Bytes*/
	if ( fifoThrlevel )
	{
	...
	for ( i = 0 ; i < fifoWriteItrCount ; i++ )
	{
		...
		{
			if ( numofBytes == 512 )
				status = MMCSD_sendCmd( 0x12800
						| MMCSD_WRITE_BLOCK,
						cardMemAddr, 0,
						MMCSD_STAT0_RSPDNE );
			else
				status = MMCSD_sendCmd( 0x12880
						| MMCSD_WRITE_MULTIPLE_BLOCK,
						cardMemAddr, 0,
						MMCSD_STAT0_RSPDNE );
		}
