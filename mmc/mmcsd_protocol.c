...
{
	return MMCSD_sendCmd( ( 0x4000 | MMCSD_GO_IDLE_STATE ), 0, 0,
			      MMCSD_STAT0_RSPDNE );
}

/*
 *  Send command to read  the Card Identification( CID )register, on the MMC/SD
 */
...
/*
 *  Send command to read  the Card Specific Data( CSD )register, on the MMC/SD
 */
...
/*
 *  Send command to say the next command is an application specific command
 */
...
//            return E_INVALID_INPUT;
	...
	if ( MMCSD_setDataSize( 1, blklength ) )/* blks = 1, bytes = blklen */
		return E_FAIL;
	...
	if ( MMCSD_sendCmd( 0x12000 | MMCSD_READ_SINGLE_BLOCK,
			    cardMemAddr, 0, MMCSD_STAT0_RSPDNE ) )
		return E_FAIL;
...
//            return E_INVALID_INPUT;
	...
	status = MMCSD_setDataSize( 1, blklength ); /* blks=1, bytes = blen */

	status = MMCSD_sendCmd( MMCSD_SET_BLOCKLEN, blklength, 0,
				MMCSD_STAT0_RSPDNE );

	if ( status == 0 )
	{
		/* Write Data, every time MMCDXR Reg full */
		status = MMCSD_writeNWords( ( Uint32* )src, blklength,
					    cardMemAddr );
...
/*
 *  command to check the Operating Voltage Conditions matches that of the host
 */
Uint16 MMC_sendOpCond( Uint32 voltageWindow, Uint32 opTimeOut )
{
	for ( ; opTimeOut > 0 ; opTimeOut -- )
	{
		...
		MMCSD_sendCmd( MMC_SEND_OP_COND, voltageWindow, 1,
			       MMCSD_STAT0_RSPDNE );
...
Uint16 SD_sendRCA( Uint32* rcaRcvd )
{
	...
	/*
	 *  rca is given a value >= 2. 0 or 1 ( in case of SanDisk cards ) is
 ...
 */
Uint16 MMCSD_cardIdentification( MMCSD_ConfigData* config, Uint32* rca,
				 MMCSD_cardStatusReg* cardStatus,
				 Uint32 opTimeout )
{
	Uint16 status;
 ...
 RESEND_CMD41:
	...
	/* Return to the application, if the card is MMC or SD card */
		       ...
		       MMCSD_ConfigData* config, Uint16 fifoThrlevel )
{
	...
	MMCSD_MMCCLK = 0x0104;     // Enable the Clock and set the divide value

	for( i = 0 ; i < 1000 ; i++ );

	if ( fifoThrlevel == MMCSD_FIFOLEVEL_32BYTES )  //  FIFO lev: 16 or 32
	...
	return MMCSD_cardIdentification( config, relCardAddr, cardStatus,
					 MMCSD_OP_TIMEOUT );
}
